class CustomDeviseController < ApplicationController
	...
	def configure_permitted_parameters
		if resource_name == :student
			devise_parameter_sanitizer.permit(:sign_up, keys: [:username, :first_name, :last_names, :remember_me])
			devise_parameter_sanitizer.permit(:account_update, keys: [:username, :first_name, :last_names, :remember_me])
